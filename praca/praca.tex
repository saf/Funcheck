...
Section \ref{sec:modelling}, we will introduce a~way of mapping the
concepts from Jimuva into the syntax of Java. Next, in Section
\ref{sec:properties}, we will define the~properties verified by
JimuvaChecker.
...
\item \texttt{C<rd, v> x} -- \texttt{x} is \emph{read-only}: its state
  cannot change throughout the lifespan of~the~ob\-ject.
...
occurrence of a type. We~might, for~example, constrain the access
rights for a method parameter or the returned value, as shown in the
following listing:
\begin{lstlisting}
public class B {
...
\item \textbf{JimuvaFlow}: performs \emph{data-flow analysis} on the
  ...
  explicitly included by the user. The annotations are
  par\-ti\-cu\-lar\-ly important in method anonymity checking, which
  is further described in Section \ref{sec:chk:anon}.
...
\begin{invariant}[Encapsulation of representation] \label{inv:encap}
  ...
  $w$'s state, provided that such static references did not exist
  prior to assigning the objects to the fields.
\end{invariant}
...
creating type systems for~ve\-ri\-fi\-ca\-tion of~object ownership and
