...
\todo{About the thesis}

\chapter{Object immutability}

\todo{More general information on immutability, motivation}
...
JSR 308 is a specification request which concerns an extension to the
...
\paragraph{Important note: \texttt{@Immutable} is not \texttt{final}} \crlf
...
proceeds to describe the main concepts of the framework. Section
\ref{sec:checking} contains detailed information on the type system
and verification implemented in JimuvaChecker.
...
values -- annotated with \texttt{@Peer} or \texttt{@Rep} may be passed
as safe parameters to methods. Therefore, elements annotated by
...
methods. In that case, the subtyping rule will be of no use, as there
will be no ``requirements'' present on the method parameters. 
...
\begin{verrule}[Indirect safety]
  A safe value may never be passed as a non-safe parameter to a method. 
\end{verrule}
...
(page~\pageref{lst:stack-peer}), but imagine that the \texttt{head}
field and the \texttt{Cell<T>} class are public:
\begin{lstlisting}
...
typecheck and would produce a static alias to an object which is owned
by the owner of \texttt{s} and should be encapsulated. The violation
...
\todo{Conclude on the checker, explain why invariants hold}
...
\todo{User reference for the error messages printed by JimuvaChecker}
