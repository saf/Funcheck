...
object-oriented programming and program verification. For example,
using immutable objects in a multi-threaded environment is obviously
safe. Section \ref{sec:imm-general} provides more examples on how
immutable objects contribute to simplifying the reasoning about
Java programs.
...
the immutability of particular objects in a limited, Java-like
language extended with object immutability and ownership
specifications.

The objective of our work is to adapt the ideas introduced in Jimuva
so that they apply to the full Java language. Therefore, a checker
called JimuvaChecker was created, which verifies properties related to
object immutability and ownership. Basically speaking, the checker
introduces the concept of \emph{immutable objects} and \emph{immutable
  classes} whose instances are immutable objects only. The eventual
...
\item[Chapter \ref{chap:related}] provides some information on other
  approaches to verifying object and reference immutability. 
\end{description}
This paper also contains two appendices:
\begin{description}
\item[Appendix \ref{chap:appendix}] contains a list of error messages
  issued by the checker for user reference. 
\item[Appendix \ref{chap:cd}] lists the contents of the CD attached to
  this work.
\end{description}
...
This section will present some general information related to the
notions of object immutability and ownership and present Jimuva -- the
type system for verifying immutability which this work is based
upon. Then, in Section \ref{sec:modelling}, we will introduce a way of
mapping the concepts from Jimuva into the syntax of Java. In Section
\ref{sec:properties}, the properties verified by JimuvaChecker will be
defined. 
...
fact include: 
\begin{itemize}
\item \textbf{Object invariants.} Suppose we want to keep an invariant
  related to the state of an object. Naturally, for immutable objects
  the state does not change in an observable way, and therefore no
...
\item \textbf{State-space reduction.} This point is related to the
  ...
  programs by traversing all possible states). When considering
...
Haack, Poll, Shäfer and Schubert in \cite{jml-imm}. Our approach to
...
There are two basic flavours of object ownership:
\begin{itemize}
...
parameters, but disallow creating static aliases to the objects. An
important feature of our ownership type system is that an object may
be owned by almost any other object within its scope.

Several previous approaches to implement various flavours of object
...
Owner specifications may include an identifier of an object from the
...
used to modify the state of \texttt{this}. Section \ref{sec:chk:safe}
...
their code, though they may want to emphasize the mutability of a
particular object.
...
\end{lstlisting}
This annotation is equivalent (and internally translated by
JimuvaChecker) to annotating the receiver type of the method with
\texttt{@Immutable}.
...
result, the \texttt{wrlocal} annotation is unnecessary, because all
...
annotation introduces \texttt{references}, rather than particular
objects, as owners.
...
foreign object to pass an appropriate argument, because the requested
...
Several example propagation rules are shown below in Hoare logic:
\begin{prooftree}
  \AxiomC{v : $\tau$}
  \RightLabel{(assign)}
...
\begin{verrule}[Encapsulation of representation]
  ...
  \item $v$ cannot be passed as a non-safe parameter to methods of
    ...
    the data-flow analysis described in Section~\ref{sec:chk:anon}
...
declared variables. This stack, as well as the methods and classes
enclosing the place of code being inspected, are scanned for a
...
that immutable classes are not illegally instantiated, i.e. all their
...
The only approach to verifying the deterministism of Java methods we
...
prove that Java methods are deterministic.
...
sufficient expressiveness of the language and the neccessary
...
limited to \texttt{this} and \texttt{world} (apart from those, Jimuva
has \texttt{peer} and any object identifier). However, their language
is expressive enough -- it may for example express iterators --
because of the owner-polymorphism which allows for more than one
ownership parameters for classes and methods. Let us look at an
implementation skeleton of a stack holding values of type \texttt{T}.
...
structure for Java programs. The ownership type system allows the user
to specify multiple ownership domains for one object. To overcome the
restrictions imposed by encapsulation of representation, ownership
domains can be \emph{linked} so that one can access the other across
domain boundaries. Both Universes and ArchJava are expressive enough
to allow implementing iterators and other important design patterns. 
...
using the Checkers Framework, with the additional keywords being
...
\item \textbf{Ownership Generic Java (OGJ)} -- a type system for
  ...
  \item \texttt{Object<M>}, where \texttt{M} is a static owner
    ...
    thus owned by the package and not any object allocated on the
...
Checkers Framework and, like the Javari checker, they are shipped
along with the framework.
...
The following list contains a reference for all error messages issued
by JimuvaChecker, with an additional explanation if needed. 
