...
formulation of the property, which will be pursued in this work, is
that the state of $P$ cannot be changed changed at all throughout its
...
important to the current work. Please refer to~\cite{haack} for a
...
introduced below are reflected in~the~JimuvaChecker type
...
JimuvaChecker. Users may specify the access rights of their objects
...
\texttt{@Immutable}, JimuvaChecker enforces that the value returned
...
JimuvaChecker, we prohibit all objects from directly accessing the
...
verified by JimuvaChecker. We have to distinguish between
...
by JimuvaChecker. The checker works by applying the type inference
...
is a subtype of $T$. JimuvaChecker makes use of this functionality,
...
JimuvaChecker utilizes \emph{data-flow analysis} to infer information
...
$x : \tau$ will denote that the value $x$ is of type $\tau$. Moreover,
$x : \lbrace\tau_1, \tau_2\rbrace$ will be used as an abbreviation for
...
\item In (nthis-member) and in (nthis-inv), we assume that a reference
  ...
  to a field. Notice, however, that this does no~harm
  to~the~immutability properties, because anonymity is only crucial
  during construction.
...
along with the~va\-lu\-es. JimuvaChecker takes a conservative
approach, possibly coming up with spurious errors, but ensuring that
the immutability property holds. The propagation rules which handle
...
\texttt{a} does not leak the reference to its internal representation
object \texttt{a.o} to the outside.  

The rest of this section will detail on how the above invariant, and
the immutability invariant with regards to deep state of objects, are
enforced.
...
hand, the type system used by~JimuvaChecker is elastic enough to allow
